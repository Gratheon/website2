...
## Added by autonomous analysis (2026-07-10, pass 7)

### Security and API Hardening (💛 High)
- [ ] 💛 Cross-service CORS hardening - replace wildcard CORS in `swarm-api` (`AllowedOrigins: ["*"]` + credentials), `telemetry-api` (`cors.AllowAll()`), and `gate-video-stream` REST (`origin: '*'`) with env-driven allowlists for app, website, Tauri, and local dev origins; add tests for denied origins. **Impact:** Reduces browser-token exposure and production attack surface. **Effort:** Low-Medium.
- [ ] 💛 Request body and upload limit guardrails - add `http.MaxBytesReader` to `telemetry-api` REST ingestion, Fastify body limits to `gate-video-stream` raw frame endpoints, and centralized GraphQL upload limits/errors for `image-splitter` / `gate-video-stream`; avoid unbounded `io.ReadAll` and wildcard raw parser buffering. **Impact:** Prevents memory exhaustion from large uploads or malformed edge traffic. **Effort:** Low-Medium.
- [ ] 💚 Secrets and token redaction audit - ensure request/context logs never include `token`, `authorization`, API keys, DB passwords, `publishToken`, or `signalingToken`; add redaction helpers in `log-lib` / `log-lib-go` and regression tests for `graphql-router`, `image-splitter`, `gate-video-stream`, and `entrance-observer`. **Impact:** Compliance and safer production debugging. **Effort:** Medium.
- [ ] 💚 Device API token rotation flow - extend device registration with scoped, revocable tokens, last-used audit log, one-click rotation, and forced re-provisioning guidance for `entrance-observer` / sensor nodes. Complements device registry but focuses on credential lifecycle. **Impact:** Limits blast radius of lost edge devices. **Effort:** Medium.

### Reliability and Operations (💚 Medium)
- [ ] 💚 Standardize health/readiness endpoints - align services on `/health` (process), `/ready` (DB/Redis/S3/schema registry), and `/metrics`; `swarm-api` lacks a public health route, `gate-video-stream` exposes metrics but no readiness, and `image-splitter` `/healthz` should return 503 when dependencies are disconnected. **Impact:** Safer deploys and load-balancer probes. **Effort:** Low-Medium.
- [ ] 💚 Database migration safety pipeline - add CI that applies all migrations from scratch and from previous release snapshots for MySQL/Postgres services (`swarm-api`, `user-cycle`, `image-splitter`, `telemetry-api`, `alerts`, `gate-video-stream`); document rollback/forward-fix strategy per service. **Impact:** Prevents production deploy failures from broken migrations. **Effort:** Medium.
- [ ] 💚 Production backup and restore drills - define and automate restore tests for MySQL, Postgres/Timescale, S3/Minio frame images, entrance video segments, and Redis-derived queues; record RPO/RTO and run quarterly drills. **Impact:** Disaster recovery confidence for customer colony history. **Effort:** Medium-High.
- [ ] 💚 Edge offline spool replay guarantees - harden `entrance-observer` local `metrics.jsonl` / video chunk storage with bounded queue size, dedupe IDs, exponential replay after reconnect, and web-app visibility for delayed data. **Impact:** Prevents data gaps in weak apiary connectivity. **Effort:** Medium.

### Quality, Accessibility and Developer Experience (💚 Medium)
- [ ] 💚 Accessibility regression suite for field workflows - add axe/Playwright checks for hive inspection, frame upload, alerts settings, device setup, maps, and high-contrast/glove-friendly mobile states; include keyboard navigation and screen-reader labels. **Impact:** Better field usability and compliance readiness. **Effort:** Medium.
- [ ] 💚 API contract and schema drift tests - generate contract snapshots for `telemetry-api` OpenAPI, `gate-video-stream` OpenAPI, and federated GraphQL SDL; fail CI when route/schema changes are undocumented or not reflected in docs/examples. **Impact:** Safer integrations for edge devices and external partners. **Effort:** Medium.
- [ ] 💚 Generated JS `app/` directory policy - `gate-video-stream`, `image-splitter`, and `user-cycle` commit compiled `app/` next to TypeScript `src/`; decide whether to remove generated output from git or add a CI drift check (`npm run build && git diff --exit-code app`). **Impact:** Prevents stale runtime code and confusing reviews. **Effort:** Low-Medium.
- [ ] 💚 Dependency and supply-chain automation - add Dependabot/Renovate, lockfile refresh cadence, SBOM/license scan, and vulnerability triage labels across Node/Go/Python repos; include GitHub Actions permission hardening for deploy workflows. **Impact:** Security hygiene across the multi-repo platform. **Effort:** Medium.
