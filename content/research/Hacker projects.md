...
| ⭐️ Hive-Cam | TobysWorkshop | [GitHub](https://github.com/TobysWorkshop/Hive-Cam) | Lightweight computer vision app and YOLOv8 model for honeybee tracking and counting. It lets a user upload hive-entrance video, automatically identifies and tracks bees flying in and out, and plots foraging trends over time. Verified as a close Gratheon analogue because it targets the same entrance-traffic counting problem, but it was not present in this list before. |
| ⭐️ BeeAlarmed | BeeAlarmed project maintainers | [GitHub](https://github.com/BeeAlarmed/BeeAlarmed), [YouTube example](https://www.youtube.com/watch?v=sRm5TmTcnpg&t=0s) | Camera-based hive monitor originally designed for Jetson Nano. It identifies and tracks bees, counts bees entering or leaving, detects pollen loads, wasps, cooling behavior, and varroa-infected bees, then can send results through LoRaWAN. Verified as similar because it combines entrance tracking with health/threat classification. |
| ⭐️ Hive Guard Bee Density Analyzer | SheikhaHassan | [GitHub](https://github.com/SheikhaHassan/Beekeepers) | Streamlit web app for uploaded beehive videos: the beekeeper marks the entrance and guard-density zones, then receives a downloadable Excel activity log with timestamps for entering, exiting, and pollen-carrying bees. Similar to Gratheon because it converts entrance video into structured bee activity records rather than only showing detections. |
| ⭐️ Bee-Nest-Computer-Vision | iurimb | [GitHub](https://github.com/iurimb/Bee-Nest-Computer-Vision) | Proof-of-concept computer-vision pipeline for bee nest analysis using YOLOv8, ByteTrack, Supervision, a custom dataset, and line-zone counting of bees going in and out. Verified as relevant because it explicitly tackles small fast bees, tracking, and directional entrance counts with reproducible scripts and result videos. |
| ⭐️ HiveWatch | mulosbron | [GitHub](https://github.com/mulosbron/HiveWatch) | Python/YOLO11x project for automated detection of bees, wasps, and other hive threats with a custom dataset workflow, visualization, model training, evaluation, comparison, and image tracking folders. Similar to Gratheon because it focuses on visual hive activity and threat detection at the apiary, not generic environmental telemetry. |
| ⭐️ GoBees | David Miguel Lozano, University of Burgos | [GitHub](https://github.com/davidmigloz/go-bees), [ReadTheDocs](http://go-bees.readthedocs.io/), [Google Play](https://play.google.com/store/apps/details?id=com.davidmiguel.gobees) | Archived Android app that uses a phone camera to monitor bee flight activity at the hive entrance, manage apiaries and hives, log weather, and analyze activity data. Verified as non-duplicate and valuable because it explores a smartphone-as-sensor workflow instead of dedicated Raspberry Pi or edge-AI hardware. |
