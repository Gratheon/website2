...
We are designing **Hive Tower**, a robotic cabinet around a standard vertical beehive. It inspects the colony without a beekeeper opening the hive:
- it lifts the boxes above the one being inspected, cracking the propolis seal one edge at a time
- it lifts each frame straight up and photographs both faces straight on, so cameras can see into the cells
- it sets each frame down in the free gap beside it, like a beekeeper working through a box, and closes the hive with a rolling landing so bees are not crushed
- it uploads the frame photos to the [web app](../web_app/index.md) for AI analysis and keeps a copy locally
- later, the same robot could move on wheels between hives (see [Robotic Apiary](../robotic_apiary/robotic_apiary.md))

`status`: concept phase, [TRL 2](https://www.nasa.gov/directorates/somd/space-communications-navigation-program/technology-readiness-levels/)
...
<div class="hive-tower" data-hive-tower>
  <div class="ht-stage">
    <canvas class="ht-canvas" data-ht="canvas" aria-label="3D model of the Hive Tower robotic beehive"></canvas>
    <div class="ht-hint">
