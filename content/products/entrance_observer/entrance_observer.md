...
heroImageAlt: Entrance Observer concept on a hive, a house-shaped wall frame with a steep opal roof, a yellow-faced camera pod under the ridge, an entrance porch with an automatic gate and a grey landing board with calibration markers
...
### 3D model
This is the Phase 3 production kit concept: a 4K camera under a small steep roof over the hive entrance. A short porch moves the doorway onto a calibrated landing board, so every bee crosses the counting line in view, even if it landed on the hive wall. An automatic gate in the porch mouth narrows the entrance against wind and robbing and switches to a hornet guard within seconds of a hornet showing up. Everything hangs on a flat wall frame, and only the camera + compute pod is machined metal. Drag to orbit, point at a part to see what it does, and press *Explode* to take it apart. The inset in the corner shows what the camera sees, and *Entrance gate* moves the gate between its four positions. *Hangs on* moves the same Observer onto the [beehive scale](../scales/scales.md) or the [Robotic Beehive](../robotic_beehive/robotic_beehive.md), and *Power → Solar* shows the solar roof. The design rationale and energy budget are in [DESIGN.md](https://github.com/Gratheon/entrance-observer/blob/main/docs/DESIGN.md).
...
<div class="entrance-observer" data-entrance-observer>
  <div class="eo-stage">
    ...
    <div class="eo-inset-label" data-eo="inset-label">Camera view · 3840 × 2160, tilted toward the hive</div>
  ...
  <aside class="eo-panel">
    ...
    <section>
      ...
      <div class="eo-controls">
        ...
        <div class="eo-field eo-wide">
          <span>Entrance gate (moves on its own; try it)</span>
          <div class="eo-seg" role="group" aria-label="Entrance gate position" data-eo="gate">
            <button type="button" data-value="open" aria-pressed="true">Open</button>
            <button type="button" data-value="reduced" aria-pressed="false">Reduced</button>
            <button type="button" data-value="guard" aria-pressed="false">Hornet guard</button>
            <button type="button" data-value="closed" aria-pressed="false">Closed</button>
          </div>
        </div>
        <div class="eo-field">
          <span>Power</span>
