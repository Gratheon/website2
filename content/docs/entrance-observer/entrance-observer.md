...
flowchart LR
  ...
  subgraph Cloud[Gratheon cloud]
    video[gate-video-stream video gateway]
  ...
  graphql --> video
  video -->|session command| uploader
  uploader -->|publish only while viewed| video
  video --> player
...
5. **Start live stream on demand** - when a beekeeper opens a hive section and requests live video, `web-app` starts a short-lived session through `gate-video-stream`. The Jetson publishes only while the session is active, using outbound control/media connections to the cloud video gateway.
...
| `gate-video-stream` | GraphQL/REST/OpenAPI plus media relay | Own video sessions, authorize short-lived live streams, route media through the cloud, accept selected entrance videos, serve HLS playback playlists, and retain training/debug clips. |
