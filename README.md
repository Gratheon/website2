...
REST API specifications and Swagger UI pages are hosted by the microservices themselves, for example `https://telemetry.gratheon.com/openapi.json` and `https://telemetry.gratheon.com/docs/`. Keep OpenAPI contracts in the service repositories instead of committing generated API artifacts into this website repo.
...
Production keeps only the active immutable release under `/www/website/releases`. During a deploy, the new release reuses unchanged files from the active release via hardlinks when `rsync` is available; after the atomic `current` symlink switch succeeds, the old release is removed. Release pruning also runs before the build and after failed builds so stale output cannot permanently fill the server. Set `KEEP_RELEASES` to a larger positive integer for temporary rollback retention. The deploy script symlinks the build `.cache` directory to `/www/website/shared/cache` so processed image variants survive between deploys; this avoids re-encoding hundreds of large images on every run and keeps memory use low on the self-hosted runner. Image processing is intentionally configured with one parallel worker and a 1600px full-size variant to keep the first uncached deploy below typical small-runner memory limits.
